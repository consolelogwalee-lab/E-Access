...
// Inbox: real conversations only.
export async function GET() {
  const user = await currentUser();
  if (!user) return NextResponse.json({ error: "Not signed in." }, { status: 401 });
  return NextResponse.json({ threads: await listInbox(user.id), meId: user.id });
}

// Start (or reopen) a conversation with another user. Body: { peerId, listingId?, body? }
export async function POST(req: Request) {
  ...
  if (body) await postMessage(id, user.id, user.full_name, body);
