...
export async function PATCH(req: Request) {
  ...
  const owner = await q1<{ owner_id: number; title: string; verification_status: string }>(
    "SELECT owner_id, title, verification_status FROM listings WHERE id = $1", [listingId]
  );
  if (owner?.owner_id) {
